  ...
  /****  Result:  Number of errors                  ****/
{
  ...
  while (phonet_rules[i] != PHONET_END)
  {
    ...
    if (k >= 0)
    {
      ...
      if (k > 0  &&  i % 3 != 0)
      {
        Rprintf (err_text,"  Meta char in replacement string");
        k = -10;
      }
      else if (k <= -100)
      {
        Rprintf (err_text,"  Lower-case letter in string");
      }
      else if (k < 0)
      {
        Rprintf (err_text,"  Syntax error in search string");
      }
      else if ((int) strlen (phonet_rules[i]) > 30)
      {
        Rprintf (err_text,"  String very long ( > 30 chars)");
        k = -1;
      }
      s = phonet_rules[i];
    ...
    if (k >= 0  &&  i % 3 != 0)
    {
      ...
      while (*r != ')'  &&  *r != '\0')
      {
        ...
        /****  do the check  ****/
        Rprintf (orig, "%c%s", TEST_char, phonet_rules[i-n]);
        Rprintf (orig2, "%c%s", TEST_char, phonet_rules[i]);
        ...
        {
          Rprintf (orig, "%s%c", orig, TEST_char);
          Rprintf (orig2,"%s%c", orig2,TEST_char);
        }
              ...
              ||  strcmp (text2,orig2) != 0)
        {
          ...
          if (*r != ' ')
          {
            Rprintf (orig," for '%c'", *r);
          }
          Rprintf (err_text, "  result %d%s: \"%s\"%s\"",
                   n,orig, text,text2);
